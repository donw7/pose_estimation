...
% source video paths
vidrootpath = sleapPath; % assume in same directory as analysis h5 files
...
params.nodeidxSB = 4; % if scope, otherwise can customize nodes
...
params.distpx = 3000; % set high to include all fr, low to isolate social proximity frames
...
cd(vidrootpath) % default example, can customize as above
dvidfiles = dir('*.mp4');
...
%% write back to h5
% modify existing so can put back into gui and relabel (create - str process error)
